...
.dseg
RIDH0: .byte 1			; received message ID higer portion storage area
RIDL0: .byte 1			; received message ID lower portion storage area
DLC0:  .byte 1			; data length count buffer0 information stored here
data1: .byte 1			; 1st byte of data
data2: .byte 1			; 2nd byte of data
data3: .byte 1			; 3rd byte of data
data4: .byte 1			; 4th byte of data
data5: .byte 1			; 5th byte of data
data6: .byte 1			; 6th byte of data
data7: .byte 1			; 7th byte of data
data8: .byte 1			; 8th byte of data
TECounter: .byte 1		; transmit error counter value
RECounter: .byte 1		; receive error counter value
EFLAG: .byte 1			; error flag
statereg: .byte 1		; state register (stores receive flag,RTR etc user defined)
RTRHI: .byte 1			; RTR ID high
RTRLO: .byte 1			; RTR ID low
DLC1: .byte 1			; data length count buffer1 information stored here
RIDH1: .byte 1			; received message ID higer portion storage area buffer1
RIDL1: .byte 1			; received message ID lower portion storage area buffer1


U1:  .byte 1				; U1 to U34 is like transmit buffer for UART , message id, DLC,8 data bytes in ascii format takes 34 bytes
U2:  .byte 1
U3:  .byte 1
U4:  .byte 1
U5:  .byte 1
...
loop1:
		...
		cpi temp,1						; compare r16 to 1
		breq LED_ON1					; if r16 is 1 branch to LED_ON1 
		cpi temp,0						; if r16 is 0
		breq LED_OFF1					; branch to LED_OFF1
LED_ON1:
		...
		rcall TXUSART					; transmit USART messages to terminal for debugging
		rcall ms1000					; delay 1 second
		rjmp loop1						; loop again
LED_OFF1:
		rcall LED_OFF					; call subroutine to switch off LED on PB3
		rcall CAN_RECEIVE				; check for new message in RXBUF0
		rcall convertaddress			; convert received id by shifting right 5 places 0x2000 bec0mes 0x100 and 0x0060 becomes 0x003
		rcall POINTER					; this OR s the value stored in SRAM registers with 0x30 and stores in UART display buffer
		rcall TXUSART					; transmit USART messages to terminal for debugging
...
MHZ10:									; routine to set clock speed of attainy804
...
SPI_TX:	        
		...
		rcall checkif					; if IF flag not set sit in a tight loop
...
SPI_RX:									; routine to receive data via MISO
		sts SPI0_DATA,address			; write address to SPI data register
		rcall checkif					; if IF flag not set sit in a tight loop
...
SPI_WRITE:								; routine to write to MCP2515 registers with SPI
		rcall SS_LO						; call SS_LO routine to pull SS iine low
		push address					; save address register to stack
		ldi address,WRITE				; write command for MCP2515, data will be written to internal registers through SPI
		sts SPI0_DATA,address			; load value in address to SPI hardware register to transmit
		rcall checkif					; Check SPI busy or transmission finished
		pop address						; restore address register from stack
		rcall SPI_TX					; subroutine stores data in address and data registers to MCP registers
		rcall SS_HI						; call SPI_HI routine to release SS line
		ret								; return to caller

SPI_READ:								; routine to read from MCP2515 registers via SPI
		rcall SS_LO						; call SS_LO routine to pull SS iine low
		push address					; save address register to stack
...
SPI_BIT_MODIFY:							; routine to modify certain bits of a MCP2515 register without effecting others
		rcall SS_LO						; call SS_LO routine to pull SS iine low
		push address					; save address register to stack
		ldi address,BIT_MODIFY			; load address register MCP2515 command BIT_MODIFY
		sts SPI0_DATA,address			; load value in address to SPI hardware register to transmit
		rcall checkif					; Check SPI busy or transmission finished
		pop address						; restore address register from stack
		sts SPI0_DATA,address			; write address to SPI data register
...
KBPS125:
		...
		rcall SPI_WRITE					; call routine to write to MCP2515 registers
		ldi address, CNF2				; The bit rate is set using the three registers CNF1 to CNF3
		ldi data,0xf1	 				; BLT =1 (how PS2 is calculated),SAM=1, PS1 = 6=110 (6+1) , PRSEG=2=001  ===0b11110001
		rcall SPI_WRITE					; call routine to write to MCP2515 registers
		ldi address,CNF3				; The bit rate is set using the three registers CNF1 to CNF3
...
SET_RECEIVE:							; routine to configure MCP2515 registers to receive CAN messages
		...
		ldi data,0x00					; load 0x00 in data register
		rcall SPI_WRITE					; write to MCP2515 interrupt enable register
		ldi address,RXB0CTRL			; load address of receive buffer0 control register
		ldi data,0x00					; 0x00 accept only filtered msg, 0x60 will accept all msgs,0x64 will also allow roll over
		rcall SPI_WRITE					; write to MCP2515 RXB0CTRL register
		ldi address,RXB1CTRL			; load address of receive buffer1 control register
		ldi data,0x00					; 0x00 accept only filtered msg, 0x60 will accept all msgs
		rcall SPI_WRITE					; write to MCP2515 RXB1CTRL register
		ldi address,RXB0D1				; load address of receive buffer0 1st data register
		ldi data,0x00					; load 0x00 to clear the data register in MCP2151 , this is done 8 times below to clear all 8 data registers in RXB0
		rcall SPI_WRITE					; write to MCP2515 
		inc address
		clr data
		rcall SPI_WRITE
		...
		ldi address,RXB1D1					; do same process what was done with RXB0 data registers , all RXB1 data registes are clearaed below
		ldi data,0x00
		rcall SPI_WRITE
		inc address
		clr data
...
clearflag:								; routine to clear MCP2515 interrupt flags
		ldi address,CANINTF				; load address of CAN interrupt flag register
...
SET_MASK0:								; routine sets mask for RXB0 ,allows ids with values that match the 0's in the mask
		ldi address,RXM0SIDH			; load address of RXM0SID high (higher 8 bits of 11 bit id is stored in RXM0SIDH)
		ldi data,0xdf					; mask 0xdf9f will allow (id0x100 << 5 = 0x2000),(id 0x003 << 5 = 0x0060) ,(id0x001 << 5 = 0x0020) & (id0x002 << 5 = 0x0040) to pass
		rcall SPI_WRITE					; write data to MCP2515
		inc address						; increase address to match RXM0SIDL
		ldi data,0x9f					; we are using id 0x003 and 0x100 to receive blink LED control status. id 0x003 = on signal , id 0x100 = off signal, lower byte is left shifted 5 times
		rcall SPI_WRITE					; write data to MCP2515
		ret								; return to caller

SET_MASK1:								; routine sets mask for RXB0 2nd buffer,allows ids with values that match the 0's in the mask
		ldi address,RXM1SIDH			; load address of RXM0SID high (higher 8 bits of 11 bit id is stored in RXM1SIDH)
		ldi data,mask1					; (.equ mask1 = 0xff	,;all filters are enabled)
		rcall SPI_WRITE					; write data to MCP2515
...
SET_FILTER0:							; sets message filter for receive buffer0
		ldi address,RXF0SIDH			; load address of RXF0SIDH
...
SET_FILTER1:							; sets message filter for receive buffer0 -1
		ldi address,RXF1SIDH			; load address of RXF1SIDH
...
SET_FILTER2:							; sets message filter for receive buffer1 - 0
		...
		ldi data,blockfilter			; blocks all message (.equ blockfilter = 0xff)
		rcall SPI_WRITE
		inc address
...
CAN_RECEIVE:						; procedure to receive CAN messages by reading MCP2515 registers
		ldi address,RXB0SIDH		; load RXB0SIDH register address
		rcall SPI_READ				; call SPI__READ to read value
		sts RIDH0,r1				; store the read value in SRAM from r1
		ldi address,RXB0SIDL		; load RXB0SIDL register address
		rcall SPI_READ				; call SPI__READ to read value
		sts RIDL0,r1				; store the read value in SRAM from r1
		ldi address,RXB0DLC			; load RXB0DLC register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts DLC0,temp				; store the read value in SRAM from r1
		ldi address,RXB0D1			; load RXB0D1 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data1,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D2 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data2,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D3 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data3,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D4 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data4,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D5 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data5,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D6 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data6,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D7 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data7,temp				; store the read value in SRAM from r1
		inc address					; load RXB0D8 register address
		rcall SPI_READ				; call SPI__READ to read value
		mov temp,r1
		sts data8,temp				; store the read value in SRAM from r1

		ldi address,TEC				; load transmission error counter address
		rcall SPI_READ				; call SPI_READ to read MCP2515 register value
		ldi address,REC				; load receive error counter address
		rcall SPI_READ				; call SPI_READ to read MCP2515 register value
		ldi address,EFLG			; load error flag address
		rcall SPI_READ				; call SPI_READ to read MCP2515 register value
		ldi address,EFLG			; load error flag address
		ldi data,0x00				; load data 0x00 to clear EFLAG
		rcall SPI_WRITE				; call SPI_READ to read MCP2515 register value 
		rcall clearflag				; call routine to clear interrupt flags
		ret							; return to caller
/*
RTR_PROC:
...
TXstring1:						; routine loads value stored in SRAM U1 onwards and sends out via UART engine to serial monitor
	ldi r21,35					; U1 to U34 is 34 numbers. as zero check is done before send byte 34+1 counts needed to send all 34 bytes
	ldi YL,low(U1)				; set Y pointer to SRAM U1 where CAN message with ID , DLC & 8 bytes of data + error counters etc are stored
	ldi YH,high(U1)				; set Y pointer to SRAM U1 where CAN message with ID , DLC & 8 bytes of data + error counters etc are stored
	
loop:
	ld r20, Y+					;load r20 value in locatio pointed by Y pointer
	dec r21						;decrease counter r21
	breq exit					;if null/0x00 the end of string has reached,branch to exit
	rcall sendbyte				;call routine to transmit loaded byte
	rjmp loop					;jump to loop back through the string until a null is encountered
	rcall ms10					;10 ms delay
exit:ret
...
hex_to_ascii:				; routine converts hexadecimal values to ASCII format   0xff  = ascii FF
	ld r21,X+				; X pointer set to SRAM RIDH0  
...
convertaddress:			;routine to Right shift the received ID 5 positions as the 11 bit id has high yte in temp and last 3 bits in r17 bit7,bit6,bit5
	lds temp,RIDH0		; load received ID high portion in r16
	lds r17,RIDL0		; load received ID lower portion in r17
	lsr temp			; right shift r16
	ror r17				; rotate right r17 through carry
	lsr temp			; right shift r16
	ror r17				; rotate right r17 through carry
	lsr temp			; right shift r16
	ror r17				; right shift r16
	lsr temp			; rotate right r17 through carry
	ror r17				; right shift r16
	lsr temp			; right shift r16
	ror r17				; rotate right r17 through carry
	sts RIDH0,temp		; store shifted value in r16 to SRAM RIDH0
	sts RIDL0,r17		; store shifted value in r17 to SRAM RIDL0
	ret	
